...
<?php
    ...
    $result = $conn->query("SELECT * FROM services where id = 8;");
    $prod = $result -> fetch_assoc();
    $hits = $prod["hits"] + 1;
    $conn->query("UPDATE services SET hits = ".$hits." WHERE id = 8;"); 
    $conn->close();
?>
<?php
    if(isset($_COOKIE["lastids"])){
        if(explode(",",$_COOKIE["lastids"])[0]!= 8){
            setcookie("lastids", '8'.",".$_COOKIE["lastids"],time() + (86400 * 30),'/',"esp.sujith.live"); 
        }
        
    }
    else{
        setcookie("lastids", '8', time() + (86400 * 30),'/',"esp.sujith.live");
    }
?>
